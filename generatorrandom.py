            ...
            try:
                ...
                if not turnstile_value:
                    result = TurnstileResult(
                        ...
                        status="Failure",
                        reason="Max attempts reached without receiving token",
                    )
                    self.log.failure("Turnstile value not received")
                else:
                    result = TurnstileResult(
                    ...
                    self.log.message(
                        "Cloudflare",
                        f"Captcha successfully solved: {turnstile_value[:45]}...",
                        start=start_time,
                        end=time.time(),
                    )
...
async def main():
    ...
    print(f"{len(results)} token  {args.out} to save   {elapsed_time} - in time")
